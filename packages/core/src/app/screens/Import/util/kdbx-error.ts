...
export function kdbxErrorMessage(err: unknown): string {
	...
	if (tooExpensive)
		return `This database's key-derivation settings (${tooExpensive[1]} KiB of memory over ${tooExpensive[2]} rounds) are heavier than Vautix will attempt. Lower them in KeePassXC's database settings and save it again.`;
	if (msg.includes("KDBX_KDF_TOO_EXPENSIVE"))
		return "This database's key-derivation settings are heavier than Vautix will attempt. Lower the Argon2 memory or rounds in KeePassXC and save it again.";
