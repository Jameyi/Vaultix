...
object KeepUnlockedStore {
    ...
    private fun readAutoLockMinutes(context: Context): Int {
        val prefs = context.getSharedPreferences(VautixAutofill.CAPACITOR_PREFS, Context.MODE_PRIVATE)
        val raw = prefs.getString(VautixAutofill.PREF_AUTOLOCK_MINUTES, null)
        return raw?.trim()?.toIntOrNull() ?: VautixAutofill.DEFAULT_AUTOLOCK_MINUTES
    }
    ...
    fun save(context: Context, vekB64: String) {
        ...
        try {
            ...
            prefs(context).edit()
                .putString(VautixAutofill.SESSION_PREF_CT, Base64.encodeToString(ct, Base64.NO_WRAP))
                .putString(VautixAutofill.SESSION_PREF_IV, Base64.encodeToString(cipher.iv, Base64.NO_WRAP))
                .putLong(VautixAutofill.SESSION_PREF_AT, System.currentTimeMillis())
                .apply()
        } catch (e: Exception) {
            clear(context)
        }
    ...
    fun load(context: Context): String? {
        ...
        val p = prefs(context)
        val ctB64 = p.getString(VautixAutofill.SESSION_PREF_CT, null) ?: return null
        val ivB64 = p.getString(VautixAutofill.SESSION_PREF_IV, null) ?: return null
        val at = p.getLong(VautixAutofill.SESSION_PREF_AT, 0L)
    ...
    fun clear(context: Context) {
        prefs(context).edit()
            .remove(VautixAutofill.SESSION_PREF_CT)
            .remove(VautixAutofill.SESSION_PREF_IV)
            .remove(VautixAutofill.SESSION_PREF_AT)
            .apply()
    }
    ...
    private fun prefs(context: Context) =
        context.getSharedPreferences(VautixAutofill.SESSION_PREFS, Context.MODE_PRIVATE)

    private fun sessionKey(): SecretKey = loadSessionKey() ?: generateSessionKey()

    private fun loadSessionKey(): SecretKey? {
        val ks = KeyStore.getInstance(VautixAutofill.BIOMETRIC_KEYSTORE).apply { load(null) }
        return ks.getKey(VautixAutofill.SESSION_KEY_ALIAS, null) as? SecretKey
    }

    private fun generateSessionKey(): SecretKey {
        val gen = KeyGenerator.getInstance(KeyProperties.KEY_ALGORITHM_AES, VautixAutofill.BIOMETRIC_KEYSTORE)
        val spec = KeyGenParameterSpec.Builder(
            VautixAutofill.SESSION_KEY_ALIAS,
