...
// Vautix's classic AOSP AutofillService. Runs in our own package (a dedicated :autofill
...
class VautixAutofillService : AutofillService() {
    ...
    override fun onFillRequest(
        ...
        if (structure == null || isOwnApp(structure) || !VaultReader.hasVault(this)) {
            // Never autofill into Vautix's own UI (its webview fields are served from
        ...
        val response = try {
            ...
            Log.e(VautixAutofill.LOG_TAG, "onFillRequest failed", e)
            null
        }
    ...
    private fun addDirectDatasets(
        ...
        return try {
            ...
            Log.e(VautixAutofill.LOG_TAG, "directFill failed; falling back to auth", e)
            false
        }
    }
    ...
    // Attach a SaveInfo so Android offers "Save to Vautix?" after a sign-in, when the user
    ...
    private fun offerToSaveEnabled(): Boolean {
        val prefs = getSharedPreferences(VautixAutofill.CAPACITOR_PREFS, Context.MODE_PRIVATE)
        // Default true (DEFAULT_OFFER_TO_SAVE): absent means on.
        return prefs.getString(VautixAutofill.PREF_OFFER_TO_SAVE, null)?.trim() != "false"
    }
    ...
    private fun quickTypeEnabled(): Boolean {
        val prefs = getSharedPreferences(VautixAutofill.CAPACITOR_PREFS, Context.MODE_PRIVATE)
        return prefs.getString(VautixAutofill.PREF_AUTOFILL_QUICKTYPE, null)?.trim() == "true"
    }

    // True when the request comes from Vautix itself (its own webview UI). A provider must
    ...
    // The user confirmed "Save to Vautix?". Capture the submitted username + password and
    ...
    override fun onSaveRequest(request: SaveRequest, callback: SaveCallback) {
        try {
            ...
            Log.e(VautixAutofill.LOG_TAG, "onSaveRequest failed", e)
        }
        callback.onSuccess()
    }
}
