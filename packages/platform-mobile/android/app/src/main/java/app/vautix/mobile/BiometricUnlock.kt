...
// (vautix.biometric.vek:<id>) + wrapped VEK (prefs vek_ct:<id>/vek_iv:<id>); the read
...
object BiometricUnlock {
    ...
    fun hasCachedSecret(context: Context): Boolean {
        ...
        val p = context.getSharedPreferences(VautixAutofill.BIOMETRIC_PREFS, Context.MODE_PRIVATE)
    ...
    fun unlock(activity: FragmentActivity, title: String, onResult: (Result) -> Unit) {
        ...
        val p = activity.getSharedPreferences(VautixAutofill.BIOMETRIC_PREFS, Context.MODE_PRIVATE)
    ...
    // (vautix.biometric.vek / vek_ct / vek_iv) are suffixed with the vault id.
    private fun aliasFor(vaultId: String) = "${VautixAutofill.BIOMETRIC_KEY_ALIAS}:$vaultId"
    private fun ctKey(vaultId: String) = "${VautixAutofill.BIOMETRIC_PREF_CT}:$vaultId"
    private fun ivKey(vaultId: String) = "${VautixAutofill.BIOMETRIC_PREF_IV}:$vaultId"
    ...
    private fun clearSecret(context: Context, vaultId: String) {
        context.getSharedPreferences(VautixAutofill.BIOMETRIC_PREFS, Context.MODE_PRIVATE)
