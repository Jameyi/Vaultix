...
// VautixUnlockActivity; once the VEK is loaded this reads the logins and renders the
...
class AutofillUnlockActivity : VautixUnlockActivity() {

    companion object {
        const val EXTRA_USERNAME_IDS = "app.vautix.autofill.USERNAME_IDS"
        const val EXTRA_PASSWORD_IDS = "app.vautix.autofill.PASSWORD_IDS"
        const val EXTRA_OTP_IDS = "app.vautix.autofill.OTP_IDS"
        const val EXTRA_HOSTS = "app.vautix.autofill.HOSTS"
        const val EXTRA_LABEL = "app.vautix.autofill.LABEL"
        const val EXTRA_SHOW_ALL = "app.vautix.autofill.SHOW_ALL"
    }
    ...
    private fun showList(query: String) {
        ...
        header.addView(TextView(this).apply {
            ...
            setTextColor(color(R.color.vautix_af_foreground))
            textSize = 19f
        ...
        val search = EditText(this).apply {
            ...
            setHintTextColor(color(R.color.vautix_af_muted))
            setTextColor(color(R.color.vautix_af_foreground))
            inputType = InputType.TYPE_CLASS_TEXT
            background = roundedFill(color(R.color.vautix_af_input), color(R.color.vautix_af_border))
    ...
    private fun row(login: AutofillLogin): View {
        val rowView = LinearLayout(this).apply {
            orientation = LinearLayout.HORIZONTAL
            gravity = Gravity.CENTER_VERTICAL
            background = roundedFill(color(R.color.vautix_af_card), color(R.color.vautix_af_border))
        ...
        val chip = TextView(this).apply {
            ...
            setTypeface(typeface, Typeface.BOLD)
            background = roundedFill(color(R.color.vautix_af_chip), color(R.color.vautix_af_chip))
        ...
        if (login.username.isNotEmpty()) {
            texts.addView(TextView(this).apply {
                text = login.username
                setTextColor(color(R.color.vautix_af_muted))
                textSize = 13f
                maxLines = 1
            })
        }
