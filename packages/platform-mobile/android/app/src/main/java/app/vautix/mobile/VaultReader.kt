...
object VaultReader {
    ...
    fun vaultFile(context: Context): File? {
        ...
        val flat = File(context.filesDir, VautixAutofill.VAULT_FILE)
    ...
    fun activeVaultId(context: Context): String? {
        ...
        prefs.getString(VautixAutofill.PREF_ACTIVE_VAULT, null)?.let { raw ->
            val v = try { JSONTokener(raw).nextValue() } catch (e: Exception) { null }
            if (v is String && v.isNotEmpty()) return v
        }
        val regRaw = prefs.getString(VautixAutofill.PREF_VAULT_REGISTRY, null) ?: return null
        return try {
