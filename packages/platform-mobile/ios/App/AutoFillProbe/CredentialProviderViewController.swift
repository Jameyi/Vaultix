...
private func vautixGlyphImage() -> UIImage? {
	Data(base64Encoded: vautixGlyphBase64).flatMap { UIImage(data: $0) }
}

private struct Glyph: View {
	let size: CGFloat
	var body: some View {
		if let img = vautixGlyphImage() {
...
private struct CredentialListView: View {
	...
	var body: some View {
		VStack(spacing: 0) {
			HStack(spacing: 10) {
				Glyph(size: 26)
				Text("Vautix").font(.system(size: 20, weight: .semibold)).foregroundColor(Theme.foreground)
				Spacer()
...
class CredentialProviderViewController: ASCredentialProviderViewController {
	...
	// VautixVault, compiled into both the app and this extension so they cannot drift.
	// Keep-unlocked session (VautixVault.sessionService): the VEK cached behind device-
	...
	// Vautix opened from a verification-code field (iOS 18+). Same auth-first flow as the
	...
	// The user opened Vautix for a passkey sign-in (no specific credential pre-chosen): unlock,
	...
	private func handlePasskeyUnlocked() {
		...
		if matches.count == 1 {
			...
			showError(
				...
				String(localized: "Vautix has no passkey for this site."))
		} else {
			host(
				PasskeyPickerView(
					choices: matches,
	...
	private func handlePasskeyCreateUnlocked() {
		...
		// the user would have to open Vautix before they could use what they just created. Both
	...
	private func stashPendingPasskey(_ reg: PasskeyRegistration, req: PasskeyCreate) {
		...
		defaults?.set(pending, forKey: VautixVault.pendingPasskeysKey)
	}
	...
	private func appendToPasskeyBundle(_ reg: PasskeyRegistration, req: PasskeyCreate) {
		...
		var list: [[String: Any]] = []
		if let data = defaults?.data(forKey: VautixVault.passkeyBundleKey),
		...
		defaults?.set(blob, forKey: VautixVault.passkeyBundleKey)
	}

	// Immediate-use bridge #2: register the new credential's OS identity so iOS offers Vautix for
	...
	private func loadPasskeyBundle() -> [Passkey] {
		guard
			let data = UserDefaults(suiteName: VautixVault.appGroup)?.data(
				forKey: VautixVault.passkeyBundleKey),
			let d = (try? JSONSerialization.jsonObject(with: data)) as? [String: Any],
			let iv = d["iv"] as? String, let ct = d["ciphertext"] as? String,
			let json = try? decryptWithVek(ivB64: iv, ciphertextB64: ct),
	...
	private func passcodeFallbackAllowed() -> Bool {
		...
		return UserDefaults(suiteName: VautixVault.appGroup)?
			.bool(forKey: "\(VautixVault.biometricPasscodeFallbackKey):\(vaultId)") ?? false
	}
	...
	private func unlockWithBiometric() {
		readVek(reason: String(localized: "Unlock Vautix")) { [weak self] outcome in
	...
	private func unlockWithPassword(_ password: String) {
		guard let slot = loadSlot() else {
			...
			model?.error = String(
				localized:
					"This vault has no master password, or it hasn't synced. Open Vautix, unlock it once, then retry."
			)
			return
		}
	...
	private func loadBundle() -> (iv: String, ct: String)? {
		guard let data = UserDefaults(suiteName: VautixVault.appGroup)?.data(forKey: VautixVault.bundleKey),
			let d = (try? JSONSerialization.jsonObject(with: data)) as? [String: Any],
			let iv = d["iv"] as? String, let ct = d["ciphertext"] as? String
		else { return nil }
		return (iv, ct)
	...
	private func loadSlot() -> Slot? {
		guard let data = UserDefaults(suiteName: VautixVault.appGroup)?.data(forKey: VautixVault.slotKey),
			let d = (try? JSONSerialization.jsonObject(with: data)) as? [String: Any],
	...
	private func keepUnlockedMinutes() -> Int {
		UserDefaults(suiteName: VautixVault.appGroup)?.integer(forKey: VautixVault.keepUnlockedKey) ?? 0
	}
	...
	private func bundleVaultId() -> String? {
		let id = UserDefaults(suiteName: VautixVault.appGroup)?
			.string(forKey: VautixVault.bundleVaultKey) ?? ""
				...
				let secret = String(data: data, encoding: .utf8)
			{
				...
				completion(
					.denied(
						String(
							localized:
								"Your biometric enrolment changed. Open Vautix and turn on biometric unlock again."
						)))
			} else {
