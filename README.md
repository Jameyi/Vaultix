...
[![Matrix](https://img.shields.io/badge/Matrix-%23general%3Avautix.sh-000000?logo=matrix&logoColor=white&style=flat-square)](https://matrix.to/#/%23general:vautix.sh)
[![Android version](https://img.shields.io/endpoint?url=https%3A%2F%2Fvautix.sh%2Fapi%2Fversion%2Fandroid)](https://vautix.sh/latest/android)
[![Android downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fvautix.sh%2Fapi%2Fdownloads%2Fandroid)](https://vautix.sh/latest/android)
[![Desktop version](https://img.shields.io/endpoint?url=https%3A%2F%2Fvautix.sh%2Fapi%2Fversion%2Fdesktop)](https://vautix.sh/latest/desktop)
[![macOS downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fvautix.sh%2Fapi%2Fdownloads%2Fmacos)](https://vautix.sh/latest/desktop)
[![Linux downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fvautix.sh%2Fapi%2Fdownloads%2Flinux)](https://vautix.sh/latest/desktop)
[![Windows](https://img.shields.io/endpoint?url=https%3A%2F%2Fvautix.sh%2Fapi%2Fversion%2Fwindows)](#whats-coming-next)
...
- Chromium: [Chrome Web Store](https://chromewebstore.google.com/detail/vautix/kmokhdhoggbdcgoepifeckhgbfakaknm)
- Firefox: [Firefox Add-ons Store](https://addons.mozilla.org/firefox/addon/vautix/)
- Desktop (macOS, Linux): [vautix.sh](https://vautix.sh)
- Android: [Releases](https://github.com/flythenimbus/vautix/releases)
- iOS: [App Store](https://apps.apple.com/us/app/vautix-password-manager/id6783071787)
...
The mobile apps reuse Vautix's Rust crypto core and vault format, with native OS autofill on top:

- **System AutoFill.** Vautix registers as a native OS credential provider, so your logins and one-time codes show up in the keyboard and autofill bar across apps and browsers.
...
- **It fills your browser.** Pair the app with the Vautix extension once and the panel fills the page in front of you, over an encrypted channel that never leaves your machine. Pairing only adds to the extension: it stays a standalone thing that never needs the app installed.
...
Install it the way you install everything else: a signed and notarized `.dmg` or `brew install --cask vautix` on macOS, and an APT repository, an AppImage, `.deb` and `.rpm` packages, or a Nix flake on Linux. The disk image and the AppImage update themselves; the package-manager routes update along with the rest of your system.
...
- **Passkeys.** Vautix is your own WebAuthn authenticator: create and sign in with passkeys, in the extension and on both mobile apps. Passkeys are stored as vault entries, so they sync across your devices with no vendor cloud.
- **More than logins.** Logins, payment cards, secure notes, and SSH keys, each with their own fields.
- **Encrypted backups.** Export your whole vault to an encrypted `.vautix` file whenever you want a copy in your own hands. It still needs your master password to open.
...
- **TOTP / 2FA codes.** Paste an `otpauth://` URI or bare secret and Vautix generates the six-digit codes.
...
- **Multiple vaults.** Keep more than one vault side by side and pick which to unlock when you open Vautix. Handy for sharing a device, or walling off separate sets of logins behind their own master passwords.
...
If you love KeePass, you'll feel at home: your encrypted database, your control, no cloud middleman. Vautix even imports your KDBX4 files. Where it's different:
...
- **Modern UI.** KeePass looks like it escaped from 2003 (no disrespect). Vautix is clean and fast, with dark mode and a layout that won't make you wince.

The KeePass philosophy with a browser-native and mobile-native coat of paint and autofill that works smoothly.

## AI usage disclosure

Parts of Vautix were written with AI assistance (Claude Opus), but every line was directed, reviewed, and shaped by a software engineer with over a decade of experience, the security-critical pieces especially. The AI was a fast typist, not the architect. The codebase is heavily tested, automated and manual, because for security software "it seems to work" isn't good enough.
...
- **Open an issue first for anything big.** Bug reports and small fixes can go straight to a PR. The tracker is at [github.com/flythenimbus/vautix/issues](https://github.com/flythenimbus/vautix/issues).
...
- **Found a security issue?** Please don't file it in the public tracker. Report it privately via [GitHub Security Advisories](https://github.com/flythenimbus/vautix/security/advisories) or email, so it can be fixed before it's out in the open. See [SECURITY.md](SECURITY.md) for details.

PRs that add real-site autofill fixtures or import-format coverage are especially handy.

## Support

Vautix is free and open source. If it's useful to you, a tip is hugely appreciated. 💜 Scan a code with your wallet, or copy an address below.
...
Vautix is free software, released under the GNU General Public License v3.0. See the [LICENSE](LICENSE) file for the full text. In short: use it, study it, fork it, and share it. If you distribute a modified version, pass the same freedoms along and make your source available under the GPLv3 too.
