...
| macOS, Firefox 154 | explicit `vautix.sh` | Apple Passwords | PRF ok, one tap, **synced** |
| Windows 11 25H2, Chromium | implicit (extension id) | Windows Hello | PRF ok, one tap, **device-bound** |
| Windows 11 25H2, Firefox | explicit `vautix.sh` | Windows Hello | PRF ok, one tap, **device-bound** |
...
The rpID is `vautix.sh` because that is the domain we own. WebAuthn does not verify ownership
(no DNS or `.well-known` lookup happens for a plain `rp.id`), so an unowned domain works fine
and this was briefly `vautix.app`, which we do not own. It would still have been wrong:
...
Platform keys register under a **shared explicit rpID** (`vautix.sh`) on BOTH browsers, so a
