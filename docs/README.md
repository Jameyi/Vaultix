...
| [email-aliases.md](email-aliases.md) | Plan: per-site email aliases from Addy.io / SimpleLogin / Forward Email (Fastmail postponed), why plain `fetch` reaches them from every platform while Firefox Relay and DuckDuckGo cannot be, and why an alias suggestion cannot be pre-generated the way a password is |
