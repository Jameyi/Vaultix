...
// The .vautix portable vault, end to end through the real extension: seed a vault, export a
// SELECTION from the list under a password chosen for the file, then read that file back through
// the app's own .vautix import.
//
...
async function exportSelection(popup: Page, password: string, confirm = password) {
	...
	const out = path.join(mkdtempSync(path.join(tmpdir(), "vautix-portable-")), "export.vautix");
	await download.saveAs(out);
	return { out, download, modal };
}

/** Feed a .vautix to the import flow and submit `password` at the credential step. */
async function openVautixFile(page: Page, extensionId: string, file: string, password: string) {
	await pickImportFile(page, extensionId, /Vautix/, file);
...
test("exports a selection as a .vautix the app can read back", async ({
	...
	expect(download.suggestedFilename()).toMatch(/^vautix-selection-\d{4}-\d{2}-\d{2}\.vautix$/);
	...
	await openVautixFile(back, extensionId, out, FILE_PW);
...
test("the file opens with the export password, not the master password", async ({
	...
	await openVautixFile(back, extensionId, out, STRONG_PW);
