...
// renderer, whose host (`#vautix-autofill-dropdown`) is a light-DOM element we can observe and
...
test("suggests a strong password on a signup form, then fills it and offers to save", async ({
	...
	await expect(page.locator("#vautix-corner-prompt")).toBeAttached({ timeout: 10_000 });
...
test("Unlock & Save commits the new login after unlocking, and confirms it with a toast", async ({
	...
	const card = page.locator("#vautix-corner-prompt");
